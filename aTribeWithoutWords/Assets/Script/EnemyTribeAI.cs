...
public class EnemyTribeAI : EnemyAIBase
{
    ...
    float attackTime = 0f;
    const float attackRange = 6f;   // 공격 사정거리

    // Var for Agent Rotation
    ...
    private void LookToWard(Transform targetRotation)
    {
        _direction = (waypoints[waypointIndex].transform.position - transform.position).normalized;
    ...
    protected override void Patrol()
    {
        ...
        if (Vector3.Distance(this.transform.position, waypoints[waypointIndex].transform.position) >= waypntLeftDist)
        {
            ...
            LookToWard(waypoints[waypointIndex].transform);
        }
        // waypoint에 도달한 경우
    ...
    protected override void Chase()
    {
        ...
        LookToWard(targets[0].transform);
